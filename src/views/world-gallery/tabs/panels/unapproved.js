...
export const UnapprovedTabPanel = ({ index, value, projectId }) => {
    const user = useSelector(getCurrentUser);
    const [data, setData] = useState(new Map());

    const { items, isEnd, getNext } = usePagination(getAllPhotosAndVideosByCategory(projectId, 'unapproved', user.dashboard_role, ''), {
        limit: 15
    });
