...
export const ProGameInActionTabPanel = ({ index, value, projectId, projectsList }) => {
    const user = useSelector(getCurrentUser);
    const [data, setData] = useState(new Map());
    const { items, isEnd, getNext } = usePagination(
        getAllPhotosAndVideosByCategory(projectId, 'approved', user.dashboard_role, 'progame_in_action', projectsList),
        {
            limit: 15
        }
    );
...
ProGameInActionTabPanel.propTypes = {
