...
export const FeaturedTabPanel = ({ index, value, projectId, projectsList }) => {
    const user = useSelector(getCurrentUser);
    const [data, setData] = useState(new Map());
    const { items, isEnd, getNext } = usePagination(
        getAllPhotosAndVideosByCategory(projectId, 'approved', user.dashboard_role, 'featured', projectsList),
        {
            limit: 15
        }
    );
