...
export const DisapprovedTabPanel = ({ index, value, projectId }) => {
    const user = useSelector(getCurrentUser);
    const [data, setData] = useState(new Map());

    const { items, isEnd, getNext } = usePagination(getAllPhotosAndVideosByCategory(projectId, 'disapproved', user.dashboard_role, ''), {
        limit: 15
    });
